...
El frontend de producción usa `http://192.168.1.38:8081/api/v1/`. El API permite peticiones desde el puerto 8080.
...
4. Antes de iniciar el API, revisar `api/appsettings.json`: la conexión usa autenticación integrada, por lo que la identidad del grupo de aplicaciones debe tener acceso a SQL Server y a la base de datos de producción.
...
- Ejecutar `run-agent.cmd` al iniciar sesión. El agente usa `127.0.0.1:5174`, por lo que va en cada PC que imprima, no como servicio de red.
