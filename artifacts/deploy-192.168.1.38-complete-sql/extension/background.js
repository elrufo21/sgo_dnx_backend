...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  chrome.storage.local.set(
    ...
    const target =
      tabs.find((tab) => tab.url?.includes(isObsCapture ? "/sales/obs_capture" : "/sales/html_capture/new")) ??
      tabs.find((tab) => tab.url?.includes(isObsCapture ? "/sales/obs_capture" : "/sales/html_capture")) ??
      tabs[0];
    ...
    await chrome.tabs.update(
      target.id,
      target.url?.includes(isObsCapture ? "/sales/obs_capture" : "/sales/html_capture/new")
        ? { active: true }
