...
namespace Xml
{
    public class CrearXML
    {
        ...
        private static string F2(double? value) => (value ?? 0d).ToString("0.00", CultureInfo.InvariantCulture);
        private static string F6(decimal? value) => (value ?? 0m).ToString("0.######", CultureInfo.InvariantCulture);
        ...
        private static string UnidadSunat(string? unidad)
        {
            var valor = (unidad ?? "").Trim().ToUpperInvariant();
...
<Invoice xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:cac='urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2' xmlns:cbc='urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2' xmlns:ccts='urn:un:unece:uncefact:documentation:2' xmlns:ds='http://www.w3.org/2000/09/xmldsig#' xmlns:ext='urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2' xmlns:qdt='urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2' xmlns:udt='urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2' xmlns='urn:oasis:names:specification:ubl:schema:xsd:Invoice-2'>
    ...
    <cbc:IssueTime>" + ResolveIssueTime(comprobante.HORA_REGISTRO) + @"</cbc:IssueTime>";
                ...
                bool tieneDetraccion = TieneDetraccion(comprobante);
                decimal montoPago = ObtenerMontoPago(comprobante, tieneDetraccion);
		...
		<cac:Party>
			...
			<cac:PartyLegalEntity>
                ...
                if (tieneDetraccion)
                {
                   ...
                   <cbc:Amount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.MONTO_DETRACCION) + @"</cbc:Amount>
                   </cac:PaymentTerms>";
                }
                if (comprobante.FORMA_PAGO.Equals("Contado"))
                {
                ...
                <cbc:Amount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(montoPago) + @"</cbc:Amount>
                </cac:PaymentTerms>";
                    xml = xml + @" <cac:PaymentTerms>
                <cbc:ID>FormaPago</cbc:ID>
                ...
                if (comprobante.TOTAL_DESCUENTO > 0)
                {
                    ...
                    <cbc:MultiplierFactorNumeric>" + F2(comprobante.PORCENTAJE_DES) + @"</cbc:MultiplierFactorNumeric>
                    <cbc:Amount currencyID='" + comprobante.COD_MONEDA + @"'>" + F2(comprobante.TOTAL_DESCUENTO) + @"</cbc:Amount>
                    <cbc:BaseAmount currencyID='" + comprobante.COD_MONEDA + @"'>" + F2(comprobante.TOTAL_GRAVADAS) + @"</cbc:BaseAmount>
                    </cac:AllowanceCharge>";
                }
                xml = xml + @" <cac:TaxTotal>
		<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_IGV) + @"</cbc:TaxAmount>
		<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.SUB_TOTAL) + @"</cbc:TaxableAmount>
			<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_IGV) + @"</cbc:TaxAmount>
			<cac:TaxCategory>
            	...
            	<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_ISC) + @"</cbc:TaxableAmount>
            	<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_ISC) + @"</cbc:TaxAmount>
            	<cac:TaxCategory>
            		<cbc:ID schemeID='UN/ECE 5305' schemeName='Tax Category Identifier' schemeAgencyName='United Nations Economic Commission for Europe'>S</cbc:ID>
            		<cac:TaxScheme>
                ...
                if ((comprobante.TOTAL_EXPORTACION > 0))
                    xml = xml + @"<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_EXPORTACION) + @"</cbc:TaxableAmount>
			...
			<cac:TaxCategory>
                ...
                if ((comprobante.TOTAL_GRATUITAS > 0))
                    xml = xml + @"<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_GRATUITAS) + @"</cbc:TaxableAmount>
			<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + @"'>0.00</cbc:TaxAmount>
			<cac:TaxCategory>
                ...
                if ((comprobante.TOTAL_EXONERADAS > 0))
                    xml = xml + @"<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_EXONERADAS) + @"</cbc:TaxableAmount>
			<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + @"'>0.00</cbc:TaxAmount>
			<cac:TaxCategory>
                ...
                if ((comprobante.TOTAL_INAFECTA > 0))
                    xml = xml + @"<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_INAFECTA) + @"</cbc:TaxableAmount>
			<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + @"'>0.00</cbc:TaxAmount>
			<cac:TaxCategory>
                ...
                if ((comprobante.TOTAL_ICBPER > 0))
                    xml = xml + @"<cac:TaxSubtotal>
   <cbc:TaxAmount currencyID='PEN'>" + F2(comprobante.TOTAL_ICBPER) + @"</cbc:TaxAmount>
		   <cac:TaxCategory>       				  
			  <cac:TaxScheme>
                ...
                if ((comprobante.TOTAL_OTR_IMP > 0))
                    xml = xml + @"<cac:TaxSubtotal>
			<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_OTR_IMP) + @"</cbc:TaxableAmount>
			<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_OTR_IMP) + @"</cbc:TaxAmount>
			<cac:TaxCategory>
				<cbc:ID schemeID='UN/ECE 5305' schemeName='Tax Category Identifier' schemeAgencyName='United Nations Economic Commission for Europe'>S</cbc:ID>
				<cac:TaxScheme>
	...
	<cac:LegalMonetaryTotal>
		<cbc:LineExtensionAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.SUB_TOTAL) + @"</cbc:LineExtensionAmount>
		<cbc:TaxInclusiveAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL) + @"</cbc:TaxInclusiveAmount>
		<cbc:PayableRoundingAmount currencyID='" + comprobante.COD_MONEDA + @"'>0.00</cbc:PayableRoundingAmount>
		<cbc:PayableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL) + @"</cbc:PayableAmount>
	</cac:LegalMonetaryTotal>";
                        for (int x = 0; x <= comprobante.detalle.Count - 1; x++)
            {
                Console.WriteLine(
                    $"FACTURA ITEM={comprobante.detalle[x].ITEM} UM={comprobante.detalle[x].UNIDAD_MEDIDA}"
                );
                ...
                if (comprobante.detalle[x].COD_TIPO_OPERACION == "10" || comprobante.detalle[x].COD_TIPO_OPERACION == "20" || comprobante.detalle[x].COD_TIPO_OPERACION == "30" || comprobante.detalle[x].COD_TIPO_OPERACION == "40")
		            ...
		            <cbc:InvoicedQuantity unitCode='" + UnidadSunat(comprobante.detalle[x].UNIDAD_MEDIDA) + "' unitCodeListID='UN/ECE rec 20' unitCodeListAgencyName='United Nations Economic Commission for Europe'>" + F2(comprobante.detalle[x].CANTIDAD) + @"</cbc:InvoicedQuantity>
		            <cbc:LineExtensionAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IMPORTE) + @"</cbc:LineExtensionAmount>
		            <cac:PricingReference>
			            <cac:AlternativeConditionPrice>
				            <cbc:PriceAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].PRECIO) + @"</cbc:PriceAmount>
		            ...
		            <cac:TaxTotal>
			            <cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IGV) + @"</cbc:TaxAmount>";
                        xml = xml + @"<cac:TaxSubtotal>
				            <cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IMPORTE) + @"</cbc:TaxableAmount>
				            <cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IGV) + @"</cbc:TaxAmount>
				            <cac:TaxCategory>
					             <cbc:ID schemeID='UN/ECE 5305' schemeAgencyID='United Nations Economic Commission for Europe'>S</cbc:ID>
					            <cbc:Percent>" + F2(comprobante.POR_IGV) + @"</cbc:Percent>
                        ...
                        {
                            xml = xml + @"<cac:TaxSubtotal>
				                <cbc:TaxAmount currencyID='PEN'>" + F2(comprobante.detalle[x].IMPUESTO_ICBPER) + @"</cbc:TaxAmount>
				                <cbc:BaseUnitMeasure unitCode='NIU'>" + comprobante.detalle[x].CANTIDAD_BOLSAS + @"</cbc:BaseUnitMeasure>
				                <cac:TaxCategory>
					                <cbc:PerUnitAmount currencyID='PEN'>" + F2(comprobante.detalle[x].SUNAT_ICBPER) + @"</cbc:PerUnitAmount>
					                <cac:TaxScheme>
					                  <cbc:ID schemeAgencyName='PE:SUNAT' schemeName='Codigo de tributos' schemeURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo05'>7152</cbc:ID>
					                  <cbc:Name>ICBPER</cbc:Name>
					                  <cbc:TaxTypeCode>OTH</cbc:TaxTypeCode>	
                        ...
                        {
                            xml = xml + @"<cac:TaxSubtotal>
                                    <cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].BI_ISC) + @"</cbc:TaxableAmount>
                                    <cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].ISC) + @"</cbc:TaxAmount>
                                    <cac:TaxCategory>
                                        <cbc:ID schemeID='UN/ECE 5305' schemeName='Tax Category Identifier'schemeAgencyName='United Nations Economic Commission for Europe'>S</cbc:ID>
                                        <cbc:Percent>" + F2(comprobante.detalle[x].POR_ISC) + @"</cbc:Percent>
		            ...
		            <cac:Price>
			            <cbc:PriceAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F6(comprobante.detalle[x].PRECIO_SIN_IMPUESTO) + @"</cbc:PriceAmount>
		            </cac:Price>
	            </cac:InvoiceLine>";
                    }
                }
...
<cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_IGV) + @"</cbc:TaxAmount>
	...
	<cac:LegalMonetaryTotal>
        ...
        <cbc:ID>" + comprobante.detalle[x].ITEM + @"</cbc:ID>
<cbc:CreditedQuantity unitCode='" + UnidadSunat(comprobante.detalle[x].UNIDAD_MEDIDA) + "'>" + F2(comprobante.detalle[x].CANTIDAD) + @"</cbc:CreditedQuantity>
<cbc:LineExtensionAmount currencyID ='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IMPORTE) + @"</cbc:LineExtensionAmount>
        <cac:PricingReference>
            <cac:AlternativeConditionPrice>
                <cbc:PriceAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].PRECIO) + @"</cbc:PriceAmount>
...
<cbc:RegistrationName><![CDATA[" + comprobante.RAZON_SOCIAL_CLIENTE + @"]]></cbc:RegistrationName>
    ...
    <cac:TaxTotal>
        <cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_IGV) + @"</cbc:TaxAmount>
        <cac:TaxSubtotal>
<cbc:TaxableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_GRAVADAS) + @"</cbc:TaxableAmount>
            <cbc:TaxAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL_IGV) + @"</cbc:TaxAmount>
            <cac:TaxCategory>
                <cac:TaxScheme>
                    <cbc:ID schemeID='UN/ECE 5153' schemeAgencyID='6'>1000</cbc:ID>
...
<cbc:PayableAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.TOTAL) + @"</cbc:PayableAmount>
        ...
        <cbc:ID>" + comprobante.detalle[x].ITEM + @"</cbc:ID>
        <cbc:DebitedQuantity unitCode='" + comprobante.detalle[x].UNIDAD_MEDIDA + "'>" + F2(comprobante.detalle[x].CANTIDAD) + @"</cbc:DebitedQuantity>
        <cbc:LineExtensionAmount currencyID ='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].IMPORTE) + @"</cbc:LineExtensionAmount>
        <cac:PricingReference>
            <cac:AlternativeConditionPrice>
                <cbc:PriceAmount currencyID='" + comprobante.COD_MONEDA + "'>" + F2(comprobante.detalle[x].PRECIO) + @"</cbc:PriceAmount>
        ...
        private static bool TieneDetraccion(BE.CPE comprobante)
        {
            if (comprobante == null)
            {
                return false;
            ...
            return string.Equals((comprobante.TIPO_OPERACION ?? string.Empty).Trim(), "1001", StringComparison.Ordinal)
                || !string.IsNullOrWhiteSpace(comprobante.CUENTA_DETRACCION)
                || (comprobante.MONTO_DETRACCION ?? 0m) > 0m;
        }

        private static decimal ObtenerMontoPago(BE.CPE comprobante, bool tieneDetraccion)
        {
            var total = comprobante.TOTAL ?? 0m;
            if (!tieneDetraccion)
            {
                return total;
            }

            var montoPago = total - (comprobante.MONTO_DETRACCION ?? 0m);
            return montoPago < 0m ? 0m : montoPago;
        }
    }
}
