/*
SQL consolidado ejecutado en sesion - DNX SUNAT/OSE
Fecha: 2026-07-27
Motor objetivo: SQL Server 2008 R2
Orden de ejecucion: columnas/credenciales SUNAT-OSE, resumen fecha, crearOrden/DocumentoVenta + DocumentoVentaCpeWeb.
*/


/**********************************************************************
  INICIO: 20260727_sunat_ose_sql2008.sql
**********************************************************************/
...
PRINT '2/5 Columnas requeridas por login, resumen y DocumentoVentaCpeWeb'
GO
...
UNION ALL SELECT 'DocumentoVentaCpeWeb', CASE WHEN OBJECT_ID('dbo.DocumentoVentaCpeWeb', 'U') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'DocumentoVentaCpeWeb urls CPE', CASE WHEN COL_LENGTH('dbo.DocumentoVentaCpeWeb', 'DocuXmlUrl') IS NULL OR COL_LENGTH('dbo.DocumentoVentaCpeWeb', 'DocuCdrUrl') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'uspGuardarCredencialesSunatweb', CASE WHEN OBJECT_ID('dbo.uspGuardarCredencialesSunatweb', 'P') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'uspObtenerCredencialesSunatweb', CASE WHEN OBJECT_ID('dbo.uspObtenerCredencialesSunatweb', 'P') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'uspinsertarRBweb', CASE WHEN OBJECT_ID('dbo.uspinsertarRBweb', 'P') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'uspEditarRBweb', CASE WHEN OBJECT_ID('dbo.uspEditarRBweb', 'P') IS NULL THEN 'FALTA' ELSE 'OK' END
UNION ALL SELECT 'uspValidaUsuarioweb', CASE WHEN OBJECT_ID('dbo.uspValidaUsuarioweb', 'P') IS NULL THEN 'FALTA' ELSE 'OK' END
GO

/**********************************************************************
  FIN: 20260727_sunat_ose_sql2008.sql
**********************************************************************/
GO

/**********************************************************************
  INICIO: 20260727_fix_uspResumenFecha_sql2008.sql
**********************************************************************/

/*
    Corrige uspResumenFechaweb para SQL Server 2008 R2.
...
PRINT 'Verificacion uspResumenFechaweb'
GO

EXEC dbo.uspResumenFechaweb @Data = '2026-03-01|2026-07-27'
GO

/**********************************************************************
  FIN: 20260727_fix_uspResumenFecha_sql2008.sql
**********************************************************************/
GO

/**********************************************************************
  INICIO: 20260727_fix_uspinsertarNotaB_documentoventa_sql2008.sql
**********************************************************************/

/*
Fix crearOrden on SQL Server 2008.

Root cause:
  Legacy procedures depend on dbo.DocumentoVenta keeping its old column shape.
  Los datos CPE web viven en dbo.DocumentoVentaCpeWeb para no cambiar la forma legacy de dbo.DocumentoVenta.

This script uses explicit DocumentoVenta columns and stores the web CPE snapshot
in dbo.DocumentoVentaCpeWeb.
*/
...
/**********************************************************************
  FIN: 20260727_fix_uspinsertarNotaB_documentoventa_sql2008.sql
**********************************************************************/
GO
